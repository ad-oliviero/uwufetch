...
void get_pkg(struct info* user_info) { // this is just a function that returns the total of installed packages
	...
	struct package_manager pkgmans[] =
		{{"apt list --installed 2> /dev/null | wc -l", "(apt)"},
		 ...
		 {"expr $(find $(brew --cellar) -maxdepth 1 -type d 2> /dev/null | wc -l | awk '{print $1}') + $(find $(brew --caskroom) -maxdepth 1 -type d 2> /dev/null | wc -l | awk '{print $1}')", "(brew)"},
...
#else
	FILE* os_release  = fopen("/etc/lsb-release", "r"); // os name file
	if (!os_release) os_release = fopen("/etc/lsb-release", "read");
#endif
#ifndef __BSD__
